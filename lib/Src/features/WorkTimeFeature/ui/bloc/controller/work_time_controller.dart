...
class WorkTimeController extends GetxController{
  ...
  WorkTimeController({ this.workSpaceId, this.doctorId,this.userType=UserTypeEnum.doctor});
 ...
 final FetchDaysForCenterRepository _fetchDaysForCenterRepository = FetchDaysForCenterRepository();
 final FetchDaysForCompanyRepository _fetchDaysForCompanyRepository = FetchDaysForCompanyRepository();
  Future<void> fetchDays()async{
    ...
    if(userType == UserTypeEnum.doctor){
       response = await _fetchDaysRepository.fetchDays(workspaceId: workSpaceId!,doctorId: doctorId!);
    }else if(userType == UserTypeEnum.center){
      response = await _fetchDaysForCenterRepository.fetchDaysForCenter(workspaceId: workSpaceId!);
  ...
  final AddDayTimeForCenterRepository _addDayTimeForCenterRepository = AddDayTimeForCenterRepository();
  final AddDayTimeForCompanyRepository _addDayTimeForCompanyRepository = AddDayTimeForCompanyRepository();
  Future<void> addDayTime({required int dayId , required DayTimeModel dayTime})async{
    ...
    setLoading();
    if(userType == UserTypeEnum.doctor){
      response = await _addDayTimeRepository.addDayTime(dayId: dayId, dayTime: dayTime, workSpaceId: workSpaceId!, doctorId: doctorId!);
    }else if(userType == UserTypeEnum.center){
      response = await _addDayTimeForCenterRepository.addDayTimeForCenter(dayId: dayId, dayTime: dayTime, workSpaceId: workSpaceId!,);
    }else if(userType == UserTypeEnum.company){
      response = await _addDayTimeForCompanyRepository.addDayTimeForCompany(dayId: dayId, dayTime: dayTime,);
    }
    Get.back();
    if (response.statusCode == 200 && response.data["status"] == true) {
  ...
  final DeleteDayTimeForCenterRepository _deleteDayTimeForCenterRepository = DeleteDayTimeForCenterRepository();
  final DeleteDayTimeForCompanyRepository _deleteDayTimeForCompanyRepository = DeleteDayTimeForCompanyRepository();
  Future<void> deleteDayTime({required int dayTimeId , required int dayId})async{
    ...
    setLoading();
    if(userType == UserTypeEnum.doctor){
      response = await _deleteDayTimeRepository.deleteDayTime(dayTimeId: dayTimeId);
    }else if(userType == UserTypeEnum.center){
      response = await _deleteDayTimeForCenterRepository.deleteDayTimeForCenter(dayTimeId: dayTimeId);
    }else if(userType == UserTypeEnum.company){
      response = await _deleteDayTimeForCompanyRepository.deleteDayTimeForCompany(dayTimeId: dayTimeId);
    }
    Get.back();
    if (response.statusCode == 200 && response.data["status"] == true) {
      debugPrint("request operation success");
